...
def load_cdncnn(model_path, device=None):
    ...
    net = model.cDnCNN(in_nc=2, out_nc=1, nc=64, nb=nb, act_mode='R')  # use this if BN is not merged by utils_bnorm.merge_bn(model)
    net.load_state_dict(state_dict, strict=True)
    if not torch.cuda.is_available():
        torch.set_flush_denormal(True)
    net.eval()
